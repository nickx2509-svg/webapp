...
function Nav({ user }: { user: UserI }) {
  ...
  return (
    <div className="fixed top-4 left-1/2 -translate-x-1/2 z-50 w-[95%] max-w-6xl h-16 md:h-20 px-4 md:px-6 flex items-center justify-between bg-purple-400 rounded-2xl shadow-xl shadow-white/20">
      ...
      {/* Logo */}
      <Link href="/" className="text-xl md:text-2xl font-bold text-purple-900 shrink-0">
        Zepto
      </Link>
      ...
      <div className="flex items-center gap-2 md:gap-3">
        ...
        >
          {open ? <X className="w-5 h-5 text-purple-600" /> : user.role === 'admin' ? <Menu className="w-5 h-5 text-purple-600 lg:hidden" /> : null}
          <div className={open ? "hidden" : (user.role === 'admin' ? "hidden lg:block" : "block")}>
              <Avatar />
          </div>
        </button>
              ...
              >
                ...
                {user.role === "admin" && (
                    <div className="flex flex-col gap-1 lg:hidden border-b border-gray-50 pb-3 mb-3">
                         <p className="text-[10px] font-bold text-gray-400 px-3 uppercase mb-1">Quick Actions</p>
                         <Link href="/admin/add-grocery" onClick={() => setOpen(false)} className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-purple-50 text-gray-700 transition">
                            <PlusCircle className="w-4 h-4 text-purple-500" />
                            <span className="text-sm font-semibold">Add New Grocery</span>
                        </Link>
                        <Link href="/admin/groceries" onClick={() => setOpen(false)} className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-purple-50 text-gray-700 transition">
                            <ClipboardList className="w-4 h-4 text-purple-500" />
                            <span className="text-sm font-semibold">Manage Inventory</span>
                        </Link>
                        <Link href="/admin/orders" onClick={() => setOpen(false)} className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-purple-50 text-gray-700 transition">
                ...
                {/* User Orders */}
                {user.role !== "admin" && (
                  <Link href="/" onClick={() => setOpen(false)} className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-purple-50 text-gray-700 transition mb-1">
